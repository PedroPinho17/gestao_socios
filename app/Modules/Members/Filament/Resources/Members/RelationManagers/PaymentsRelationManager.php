...
namespace App\Modules\Members\Filament\Resources\Members\RelationManagers;
                ...
                CreateAction::make()
                    ...
                    ->mutateFormDataUsing(function (array $data): array {
                        if (blank($data['referencia'] ?? null) && filled($data['data'] ?? null)) {
                            $data['referencia'] = Carbon::parse($data['data'])->format('Y-m');
                        }

                        return $data;
                    })
                    ->after(function (Payment $record): void {
                        if (FeatureRegistry::enabled('filament.receipts')) {
                            static::sendReceiptEmail($record, automatic: true);
                        }
                    }),
            ])
            ->recordActions(array_values(array_filter([
                FeatureRegistry::enabled('filament.receipts') ? Action::make('comprovativo')
                    ->label('Comprovativo')
                    ->icon('heroicon-o-document-arrow-down')
                    ->color('success')
                    ->url(fn ($record): string => route('payments.receipt.pdf', $record))
                    ->openUrlInNewTab() : null,
                FeatureRegistry::enabled('filament.receipts') ? Action::make('enviar_email')
