...
class ClubSettingsPage extends Page
{
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('Modelos de cartão')
                    ->description('Todos os modelos existem no sistema; escolha quais aparecem no selector e na galeria de demonstração.')
                    ->columns(1)
                    ->visible(fn (): bool => FeatureRegistry::enabled('filament.cards'))
                    ->schema([
                        CheckboxList::make('card_layout.available_templates')
                            ->label('Modelos visíveis')
                            ->helperText('Para reuniões com outros clubes, pode activar só os modelos base (Clássico, Moderno, Minimal) e ocultar o layout oficial.')
                            ->options(fn (): array => collect(MemberCardLayout::catalog())
                                ->mapWithKeys(fn (array $meta, string $key): array => [$key => $meta['label']])
                                ->all())
                            ->descriptions(fn (): array => collect(MemberCardLayout::catalog())
                                ->mapWithKeys(fn (array $meta, string $key): array => [$key => $meta['description']])
                                ->all())
                            ->columns(2)
                            ->live()
                            ->afterStateUpdated(function (?array $state): void {
                                $this->syncCardTemplateSelection($state ?? []);
                            }),
                        Actions::make([
                            Action::make('onlyBaseTemplates')
                                ->label('Só modelos base')
                                ->icon(Heroicon::OutlinedSquares2x2)
                                ->color('gray')
                                ->action(fn () => $this->applyAvailableCardTemplates(
                                    MemberCardLayout::baseTemplateKeys(),
                                    'A mostrar só modelos base (Clássico, Moderno, Minimal).',
                                )),
                            Action::make('allCardTemplates')
                                ->label('Todos os modelos')
                                ->icon(Heroicon::OutlinedRectangleStack)
                                ->color('gray')
                                ->action(fn () => $this->applyAvailableCardTemplates(
                                    MemberCardLayout::allTemplateKeys(),
                                    'A mostrar todos os modelos instalados.',
                                )),
                        ]),
                        Select::make('card_layout.template')
                            ->label('Modelo activo do clube')
                            ->options(fn (Get $get): array => MemberCardLayout::templateOptions(
                                MemberCardLayout::normalizeAvailableTemplates($get('card_layout.available_templates')),
                            ))
                            ->required()
                            ->live(),
                    ]),
     ...
     */
    public function getCardPreviewGallery(): array
    {
        return app(MemberCardViewData::class)->previewGallery($this->data ?? []);
    }
    ...
    public function save(): void
    {
        ...
        $cardLayout['available_templates'] = MemberCardLayout::normalizeAvailableTemplates(
            $cardLayout['available_templates'] ?? null,
        );
        $cardLayout['template'] = MemberCardLayout::ensureTemplateAllowed(
            (string) ($cardLayout['template'] ?? 'classic'),
            $cardLayout['available_templates'],
        );
     ...
     */
    public function applyAvailableCardTemplates(array $templates, string $message): void
    {
        $this->syncCardTemplateSelection($templates);
        $this->form->fill($this->data);

        Notification::make()
            ->title($message)
            ->body('Clique em «Guardar definições» para persistir na base de dados.')
            ->success()
            ->send();
    }
     ...
     */
    private function syncCardTemplateSelection(?array $templates): void
    {
        $available = MemberCardLayout::normalizeAvailableTemplates($templates);
        $current = (string) data_get($this->data, 'card_layout.template', 'classic');

        data_set($this->data, 'card_layout.available_templates', $available);
        data_set($this->data, 'card_layout.template', MemberCardLayout::ensureTemplateAllowed($current, $available));
    }
}
